...
namespace _17825_projekat
{
    ...
    public class CryptoServer : IServer
    {
        ...
        public void FourSquareDecrypt(string text)
        {
            ...
            {
                int index = 0;
                while (FoursquareTopRight[index] != text[i]) index++;

                int x2 = index % 5;
                int y1 = index / 5;

                index = 0;
                while (FoursquareBottomLeft[index] != text[i + 1]) index++;

                int x1 = index % 5;
                int y2 = index / 5;

                char resultingChar = (char)('a' + y1 * 5 + x1);
                if (resultingChar >= 'j' - 1) resultingChar++;
        ...
        public void XXTEADecrypt(byte[] data)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine(Encoding.UTF8.GetString(BitConverter.GetBytes(decData[i])));
            }

            System.Diagnostics.Debug.WriteLine(wordNum);
